...
void loadInventoryFile(vector<Product> &inventory) {
    ...
    // The last field (dateModified) uses the default '\n' delimiter naturally.
...
void displayDashboard(const vector<Product> &inventory) {
    ...
    for (size_t i = 0; i < inventory.size(); ++i) {
        ...
        if (inventory[i].stockQuantity < LOW_STOCK) ++lowStockAlert;
    }
...
void checkAndDisplayLowStock(const vector<Product> &inventory) {
    ...
    int lowStockCount = 0;
    for (size_t i = 0; i < inventory.size(); ++i) {
        if (inventory[i].stockQuantity < LOW_STOCK) {
            cout << "  \a! WARNING: " << inventory[i].name
...
// When 'filter' is provided (called from search), matching is scoped to
// that result subset. When NULL (called from inventory menu), the full
// inventory is searched.
void deleteProducts(vector<Product> &inventory, const vector<size_t> *filter) {
    ...
    for (size_t r = 0; r < toRemoveIdx.size(); ++r)
        if (r == 0 || toRemoveIdx[r] != toRemoveIdx[r-1])
            toRemoveIdx[w++] = toRemoveIdx[r];
    toRemoveIdx.resize(w);
...
void date(string &CURRENT_DATE) {
    ...
    while (true) {
        ...
        if (tempDay >= 1 && tempDay <= 31) break;
        cout << "Error: Invalid day. Please enter a value between 01 and 31.\n";
    }

    cin.ignore(10000, '\n');

    month = (tempMonth < 10) ? "0" + to_string(tempMonth) : to_string(tempMonth);
    day = (tempDay < 10) ? "0" + to_string(tempDay) : to_string(tempDay);
