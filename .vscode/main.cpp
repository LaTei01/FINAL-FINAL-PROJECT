...
// Pagination helper
// Prevents overcrowding of data in the terminal
void displayProductsWithPagination(const vector<Product> &inventory,
...
void date(string &currentDate){
    ...
    do {
        cout << "Year (YYYY): ";
        cin >> temp;
        if (temp >= 2000 && temp <= 2100){ stringdate = to_string(temp); currentDate+=stringdate; break;}
        cout << "Invalid.\n";
    } while(true);
    currentDate.push_back('-');
    do {
        cout << "Month (MM): ";
        cin >> temp;
        if (temp >= 1 && temp <= 12){ 
            if (temp < 10) stringdate = "0" + to_string(temp);
            else stringdate = to_string(temp); 
            currentDate+=stringdate; break; }
        cout << "Invalid.\n";
    } while(true);
    currentDate.push_back('-');
    do {
...
// 'indices' holds which inventory positions to show (supports both full list
// and filtered search results). The user navigates with N/P/Q.
void displayProductsWithPagination(const vector<Product> &inventory,
                                   const vector<size_t> &indices,
                                   const string &title) {
    if (indices.empty()) {
...
void displayDashboard(const vector<Product> &inventory) {
    ...
    for (size_t i = 0; i < inventory.size(); ++i) {
        totalStockQuantity  += inventory[i].stockQuantity;
        totalInventoryValue += inventory[i].price * inventory[i].stockQuantity;
        if (inventory[i].stockQuantity < 5) ++lowStockAlert;
    }


    cout << "  Total Unique Products : " << totalUniqueItems << '\n';
    cout << "  Total Stock Volume    : " << totalStockQuantity << " units\n";
    cout << "  Total Asset Value     : P" << fixed << setprecision(2) << totalInventoryValue << '\n';
    ...
    for (size_t i = 0; i < inventory.size(); ++i) {
        if (inventory[i].stockQuantity < 5) {
            cout << "  ! WARNING: " << inventory[i].name
                 << " is low on stock! (" << inventory[i].stockQuantity << " left)\n";
            ++lowStockCount;
        }
    }
...
void displayAllProducts(const vector<Product> &inventory) {
    ...
    displayProductsWithPagination(inventory, allIndices, "Complete Inventory Table");
}
...
void searchProducts(vector<Product> &inventory, vector<Voucher> &vouchers) {
    ...
    for (size_t i = 0; i < inventory.size(); ++i) {
        const Product &p = inventory[i];
        if (changeToLowercase(p.id).find(keyLower)    != string::npos ||
            changeToLowercase(p.name).find(keyLower)  != string::npos ||
            changeToLowercase(p.brand).find(keyLower) != string::npos ||
            changeToLowercase(p.type).find(keyLower)  != string::npos) {
    ...
    cout << "  Found " << foundIndexes.size() << " matching product(s).\n";


    // Show results with pagination
    displayProductsWithPagination(inventory, foundIndexes, "Search Results");


    // After browsing, offer modify / delete options
    cout << "\n    [1]  Modify a Result\n";
    cout << "    [2]  Delete a Result\n";
    cout << "    [3]  Back to Inventory Menu\n\n";
    cout << "-----------------------------------------------------------------\n";
    cout << " Enter your choice: ";
    ...
    if (opt == 1) {
        ...
        cout << "\n To delete, enter the product ID or result number (e.g. P002 or #2).\n";
        cout << " For multiple items, separate entries with commas (e.g. P002,P003 or #1,#3).\n\n";
        cout << " Enter products to delete: ";
        string delInput;
        ...
        for (size_t i = 0; i < delInput.size(); ++i)
            if (delInput[i] == '#') { usedNumbers = true; break; }
        ...
        if (usedNumbers) {
            ...
            for (size_t i = 0; i < toRemoveIdx.size(); ++i)
                for (size_t j = i + 1; j < toRemoveIdx.size(); ++j)
                    if (toRemoveIdx[i] == toRemoveIdx[j])
                        { toRemoveIdx.erase(toRemoveIdx.begin() + j); --j; }


            // Sort descending so deletion does not mess up indexes
            for (size_t i = 0; i < toRemoveIdx.size(); ++i)
                for (size_t j = i + 1; j < toRemoveIdx.size(); ++j)
            ...
            for (size_t i = 0; i < ids.size(); ++i) {
                ...
                for (size_t j = 0; j < inventory.size(); ++j) {
                    if (inventory[j].id == ids[i]) {
                        ...
                        conf = changeToUppercase(conf);
                        if (conf == "Y" || conf == "YES") {
                            cout << " Deleted: " << inventory[j].id << " - " << inventory[j].name << '\n';
    ...
    // opt == 3 falls through and returns naturally
}
...
void updateProductByIndex(vector<Product> &inventory, size_t idx) {
    ...
    while (true) {
        ...
        switch(choice){
        case 1: cout << " New Product ID    : "; getline(cin, p.id);    cout << " Updated.\n"; break;
        ...
        case 6:
            ...
            break;
        case 7:
            p.dateModified = currentDate;
            cout << "\n Changes saved. Date modified updated to " << currentDate << ".\n";
            cout << "-----------------------------------------------------------------\n";
            return;
        default:
...
void deleteProductsInteractive(vector<Product> &inventory) {
    ...
    cout << "=================================================================\n\n";
    cout << "    [1]  Delete a single product by ID\n";
    ...
    // Delete multiple products
        ...
        cout << " Enter Product IDs separated by commas: ";
...
void voucherManagementMenu(vector<Voucher> &vouchers) {
    while (true) {
        ...
        if (ch == 1) {
            ...
            double percent;
            if (!(cin >> percent)) { cin.clear(); cin.ignore(10000,'\n'); cout << " Error: Invalid percentage.\n"; continue; }
            cin.ignore();
...
void generateReceiptFile(const string &customerName, const string &orderDate, const string &customerAddress,
    ...
    ofstream createInvoice(customerName + "_invoice.txt");
    ...
    for (size_t i = 0; i < cart.size(); ++i) {
        ...
        createInvoice << "  " << left  << setw(4)  << cart[i].quantity
            << "  "          << setw(30) << cart[i].productName
            << "  " << right << setw(9)  << fixed << setprecision(2) << cart[i].price
            << "  Php "
    ...
    // Financial summary — left label padded to match, right-aligned values
    ...
    while (true) {
        ...
        if (orderAction == 1) {
            ...
            while (true) {
                ...
                if (purchaseQuantity > inventory[foundIndex].stockQuantity) {
            ...
            if (!merged)
                temporaryCart.push_back({inventory[foundIndex].id, inventory[foundIndex].name, inventory[foundIndex].price, purchaseQuantity});
            ...
            if (checkout_confirm == "Y" || checkout_confirm == "YES") {
                ...
                string uniqueFilename = "receipt_" + customerName + ".txt";
                ...
                for (size_t ci = 0; ci < temporaryCart.size(); ++ci) {
                    ...
                    for (size_t p = 0; p < inventory.size(); ++p) {
                        if (inventory[p].id == ciRef.productId) {
                            ...
                            inventory[p].dateModified = currentDate;
                            break;
                        }
                    }
                }
                ...
                cout << "\n Transaction complete! Receipt saved as: " << uniqueFilename << "\n";
                cout << "=================================================================\n";
